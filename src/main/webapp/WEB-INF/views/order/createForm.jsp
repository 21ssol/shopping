...
<script type="text/javascript">
	function order(tot){
		let payment = document.querySelector(".btn-check:checked").value;  //클래스가 btn-check인것에서 체크되어진 하나만
		let req = document.querySelector("#req");   // 요청메세지
		let reqtext = req.options[req.selectedIndex].text;
		let f = document.querySelector("#oform");
		f.tot.value = tot;
		f.payment.value = payment;  // 히든값을 담는다.
		f.reqtext.value=reqtext;
		f.action = "/order/create/${str}";   //order컨트롤러안에 있어야한다. post방식
		f.submit();
		// alert(tot);
		// alert(payment);
		// alert(reqtext);
...
<div class="container">
  ...
  <figure>
  <img class="img-rounded" src="/contents/storage/${dto.filename}" style="width:100px; height:120px">
  ...
  <ul class="list-group">
	...
	총 주문 금액 ${tot } 원 +  배송비 3000 원 = 최종 결제금액 ${tot + 3000 } 원 
...
<form id="oform" method="post" >
	<input type="hidden" name="cno" value="${cno}">  <!-- 장바구니에서 왔을때 -->
	<input type="hidden" name="contentsno" value="${contentsno}">   <!-- 상품목록에서(장바구니 없이 주문) 왔을때 -->
